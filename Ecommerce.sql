...
select p.product_id, p.product_name, 
       (select sum(item_amount) from order_items where product_id = p.product_id) * 100.0 /
       (select sum(item_amount) from order_items) as revenue_percentage
from products p;

--Output
--product_id product_name revenue_percentage
--1	Laptop	27.906976744186
--2	Smartphone	34.883720930232
--3	Tablet	3.488372093023
--4	Headphones	6.976744186046
--5	TV	20.930232558139
--6	Coffee Maker	0.581395348837
--7	Refrigerator	NULL
--8	Microwave Oven	NULL
--9	Blender	2.441860465116
--10	Vacuum Cleaner	2.790697674418
